...
class PropaHitPattern{
   public:
      ...
      // stereo means second layer of a given hitpattern
        ...
        //defines a radius for each cylindrical hitpattern
        // Players.push_back(make_pair(1160,Cyl[0]));//PIXBL1
        // Players.push_back(make_pair(1168,Cyl[1]));//PIXBL2
        // Players.push_back(make_pair(1176,Cyl[2]));//PIXBL3
        // Players.push_back(make_pair(1184,Cyl[3]));//PIXBL4
        // Players.push_back(make_pair(1416,Cyl[4]));//TIBL1
        // Players.push_back(make_pair(1420,Cyl[5]));//TIBL1stereo
        // Players.push_back(make_pair(1424,Cyl[6]));//TIBL2
        // Players.push_back(make_pair(1428,Cyl[7]));//TIBL2stereo
        // Players.push_back(make_pair(1432,Cyl[8]));//TIBL3
        // Players.push_back(make_pair(1440,Cyl[9]));//TIBL4
        // Players.push_back(make_pair(1672,Cyl[10]));//TOBL1
        // //Currently limited to TOBL1 in the analysis
        // //defines a z for each disk/wheel
        // Players.push_back(make_pair(1288,disk[0]));//PXFdisk1
        // Players.push_back(make_pair(1296,disk[1]));//PXFdisk2
        // Players.push_back(make_pair(1304,disk[2]));//PXFdisk3
        // Players.push_back(make_pair(1544,disk[3]));//TIDWHeel1
        // Players.push_back(make_pair(1548,disk[4]));//TIDWHeel1stereo
        // Players.push_back(make_pair(1552,disk[5]));//TIDWHeel2
        // Players.push_back(make_pair(1556,disk[6]));//TIDWHeel2stereo
        // Players.push_back(make_pair(1560,disk[7]));//TIDWHeel3
        // Players.push_back(make_pair(1564,disk[8]));//TIDWHeel3stereo
        // Players.push_back(make_pair(1800,disk[9]));//TECWHeel1
        // Players.push_back(make_pair(1804,disk[10]));//TECWHeel1stereo
        // Players.push_back(make_pair(1808,disk[11]));//TECWHeel2
        // Players.push_back(make_pair(1812,disk[12]));//TECWHeel2stereo
        // Players.push_back(make_pair(1816,disk[13]));//TECWHeel3
        // Players.push_back(make_pair(1820,disk[14]));//TECWHeel3stereo
        // Players.push_back(make_pair(1824,disk[15]));//TECWHeel4
        // Players.push_back(make_pair(1828,disk[16]));//TECWHeel4stereo
        // Players.push_back(make_pair(1832,disk[17]));//TECWHeel5
        // Players.push_back(make_pair(1836,disk[18]));//TECWHeel5stereo
        // Players.push_back(make_pair(1840,disk[19]));//TECWHeel6
        // Players.push_back(make_pair(1844,disk[20]));//TECWHeel6stereo
        // Players.push_back(make_pair(1848,disk[21]));//TECWHeel7
      ...
      GloballyPositioned<float>::PositionType Main(uint16_t firsthit, AnalyticalPropagator* Prop,TrajectoryStateOnSurface tsos, float eta, float phi, float vz,Basic3DVector<float> PV,Basic3DVector<float> p )
        {
          if (firsthit==1288 || firsthit==1296 || firsthit==1304 || firsthit==1544 || firsthit==1548 || firsthit==1552 || firsthit==1556 || firsthit==1560 || firsthit==1564 || firsthit==1800 || firsthit==1804 || firsthit==1808 || firsthit==1812 || firsthit==1816 || firsthit==1820 || firsthit==1824 || firsthit==1828 || firsthit==1832 || firsthit==1836 || firsthit==1840 ||firsthit== 1844 || firsthit==1848)//supposed to be plane
            {
              return PropagateToDisk( firsthit, eta, phi, vz, PV, p );
            }
          else
            {
              return  PropagateToCylinder( firsthit, Prop, tsos, eta, phi, vz);
            }
        }
              ...
              if (Disk[i].first==firsthit)
                {
                  ...
                  else// It may happen that the propagation fails
                    {
